...
// Unico punto di conversione "HH:MM" -> {ore, minuti}: valida il formato
// prima che un valore malformato dal classificatore (o dalla risposta
// dell'utente su Telegram) arrivi a costruire una data — vedi la revisione
// finale del branch scrittura-calendario.
export function parseOra(oraStr) {
  if (!oraStr) return null;
  const match = /^([01]\d|2[0-3]):([0-5]\d)$/.exec(oraStr);
  if (!match) throw new Error(`[google-calendar] orario non valido: "${oraStr}"`);
  return { ore: Number(match[1]), minuti: Number(match[2]) };
}

export async function creaEventoCalendario({ titolo, data, ora }) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(data)) {
    throw new Error(`[google-calendar] data non valida: "${data}"`);
  }
