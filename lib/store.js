...
// Usata sia dalla cattura sia dal pannello del CRM: stesso nome, stessa
// persona — non due righe per la stessa persona scritta in modo diverso.
export async function trovaOCreaPersona(nome) {
  ...
  const creata = await addPersona({ nome: nome.trim() });
  return creata.id;
}
...
// ---------------------------------------------------------------
// Export completo — A19. Tutte le righe, tutte le tabelle, senza i
// limiti che le get* normali applicano per l'uso quotidiano.
// ---------------------------------------------------------------

export async function esportaTutto() {
  const tabelle = ["profilo", "catture", "persone", "task", "memoria", "registro", "log_giornaliero"];

  const risultati = await Promise.all(
    tabelle.map((tabella) => supabase.from(tabella).select("*").eq("user_id", USER_ID))
  );

  const dati = {};
  tabelle.forEach((tabella, i) => {
    orThrow(risultati[i].error, `esportaTutto (${tabella})`);
    dati[tabella] = risultati[i].data;
  });
  return dati;
}
