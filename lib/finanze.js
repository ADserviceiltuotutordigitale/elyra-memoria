...
// Storico per il grafico della pagina Finanze — legge dalle stesse
// istantanee salvate, mai da Odoo in diretta.
export async function leggiStoricoPolso(giorni = 90) {
  const oggi = oggiISO();
  const righe = await getLogGiornalieroRange(giorniFa(oggi, giorni), oggi);
  return righe
    .filter((r) => r.finanze && typeof r.finanze.totale_personale === "number")
    .map((r) => ({
      data: r.data,
      totale_personale: r.finanze.totale_personale,
      totale_lavoro: r.finanze.totale_lavoro,
    }));
}
